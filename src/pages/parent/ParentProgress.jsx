...
import { 
  ...
  MessageSquare, Star, ArrowLeft, Maximize2, LineChart as LineChartIcon
} from 'lucide-react';
...
const ParentProgress = () => {
  ...
  const GRADES_HISTORY = [
    { 
      ...
      teacherFeedback: "Con làm bài rất tốt, tiếp tục phát huy nhé!"
    },
    { 
      id: 'g2', 
      ...
      teacherFeedback: "An làm bài rất tốt ở phần trắc nghiệm, tuy nhiên phần tự luận còn sai sót ở bước kết luận. Con cần chú ý đọc kỹ đề bài hơn nhé."
    }
  ];
  ...
  const renderList = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Tình hình học tập</h1>
          <p className="text-gray-500 mt-1 font-medium">Xem chi tiết bài làm và nhận xét của giáo viên dành cho con.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {GRADES_HISTORY.map(grade => (
          <div key={grade.id} className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 relative overflow-hidden group hover:border-indigo-200 transition-colors cursor-pointer" onClick={() => setSelectedExam(grade)}>
            <div className="flex justify-between items-start mb-6 relative z-10">
              ...
              <div className="w-16 h-16 bg-indigo-500 text-white rounded-2xl flex items-center justify-center font-black text-2xl shadow-lg rotate-3 group-hover:rotate-6 transition-transform">
                {grade.score}
              </div>
            </div>
            ...
            <div className="relative z-10">
              ...
              <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-black text-indigo-700 uppercase tracking-wider">Lời phê của GV</span>
                </div>
                <p className="text-sm font-medium text-indigo-900 leading-relaxed line-clamp-2">{grade.teacherFeedback}</p>
              </div>
            </div>
          </div>
        ))}
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-full flex flex-col bg-gray-50/50 -m-4 lg:-m-8 p-4 lg:p-8">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6">
        ...
        <div className="text-center">
          ...
          <p className="text-xs font-bold text-gray-500">{selectedExam.subject} • {selectedExam.date}</p>
        </div>
        <div className="flex items-center gap-2 font-black text-2xl text-indigo-500 px-4">
      ...
      <div className="flex flex-col lg:flex-row gap-8 flex-1">
        ...
        <div className="flex-1 bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col relative group">
          <div className="absolute top-4 right-4 z-10">
            <button className="p-2 bg-white/80 backdrop-blur-md rounded-xl text-gray-600 hover:text-indigo-500 hover:bg-white shadow-sm transition-all">
              <Maximize2 size={20}/>
            </button>
          </div>
          <div className="flex-1 bg-gray-200 relative p-8 flex items-center justify-center overflow-auto custom-scrollbar">
            ...
            <div className="w-full max-w-2xl bg-white shadow-lg min-h-[800px] p-12 relative font-mono text-gray-800 text-lg">
              <h1 className="text-center font-bold text-xl border-b-2 border-gray-800 pb-4 mb-8">BÀI LÀM CỦA HỌC SINH</h1>
        ...
        <div className="w-full lg:w-96 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <p className="text-3xl font-black text-indigo-500 leading-none">{selectedExam.score}<span className="text-xl text-gray-400">/10</span></p>
                <p className="text-xs font-bold text-indigo-700 mt-1 bg-indigo-100 px-2 py-0.5 rounded w-fit">Rất tốt</p>
              </div>
            </div>
            
            <div className="space-y-4">
              <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 relative">
                <div className="absolute -top-3 left-4 bg-indigo-100 px-2 py-0.5 rounded-full flex items-center gap-1 text-[10px] font-black text-indigo-700 uppercase">
                  <MessageSquare size={12}/> Lời phê của GV
                </div>
                <p className="text-sm font-medium text-indigo-900 mt-2">{selectedExam.teacherFeedback}</p>
              </div>
